...
| `news_events_v2.html` | Sept 2026 — News & Events. Four photo cards, one per confirmed date (`.evgrid`, date as a plate on the frame) + a 10-frame horizontal photo rail (`.evrail`, native scroll-snap, no autoplay, no JS). **The four dates are the client's own and the weekdays they gave were checked against the calendar** — that is how the years were settled. Nothing else about the events is stated because nothing else was given. The photograph on each card is atmosphere from past events — never captioned as the date beside it. Home `#events` shows the same four and links here. |
